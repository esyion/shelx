 ...
 * 提供"前往下载"和"重新检查"两个动作。
 *
 * 不做应用内自动下载/安装(AGENTS.md "users update by reinstalling"):
 * 跳到 GitHub Releases 页,由用户自行选择适合自己平台的安装包。
 */
"use client";
...
/** 将 GitHub release body 折叠到前若干行;空内容返回占位说明。 */
...
export function UpdateDialog() {
  ...
  const body = useMemo(() => previewBody(latest?.body), [latest]);
  const isAvailable = status === "available";
  const hasError = status === "error";

  /** 跳到 GitHub Releases 页(由 opener 插件或浏览器打开)。 */
  const openReleasePage = () => {
    if (!latest?.html_url) return;
    void window.open(latest.html_url, "_blank", "noopener,noreferrer");
  };
