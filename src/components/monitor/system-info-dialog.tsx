...
export function SystemInfoDialog() {
  const sessionId = useUiStore((s) => s.systemInfoSessionId);
    ...
    const fetchOnce = (): void => {
      ...
      void fetchSystemInfo(sessionId)
        .then((data) => {
          ...
          setInfo(data);
          setError(null);
          // 字段大部分为空说明采集未完成,1s 后再拉一次直到有内容或 6 次为止。
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={(next) => {
        if (!next) setSystemInfoSessionId(null);
      }}
    >
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setSystemInfoSessionId(null)}>
            关闭
          </Button>
        </DialogFooter>
      </DialogContent>
...
/** 核心数展示;物理/逻辑都不存在 → null。 */
function formatCores(phys: number | null | undefined, log: number | null | undefined): string | null {
  const p = phys ?? null;
  const l = log ?? null;
...
/** 运行时间(秒)→ 中文格式(沿用 lib/format.formatDuration);null → null。 */
